...
def main():
    ...
    i = 0
    q = s_lite.query(ConcertSelection) \
        .options(
        joinedload(ConcertSelection.concert, innerjoin=True)
            .joinedload(Concert.orchestra, innerjoin=True),

        joinedload(ConcertSelection.concert, innerjoin=True)
            .joinedload(Concert.eventtype, innerjoin=True),

        joinedload(ConcertSelection.concert, innerjoin=True)
            .joinedload(Concert.venue, innerjoin=True),
        ...
        joinedload(ConcertSelection.movements, innerjoin=False)
            .joinedload(ConcertSelectionMovement.movement, innerjoin=True),
